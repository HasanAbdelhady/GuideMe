...
class ChatRAGFile(models.Model):
    chat = models.ForeignKey(
        Chat, on_delete=models.CASCADE, related_name='rag_files')
...
class ChatFlashcard(models.Model):
    ...
    chat = models.ForeignKey(
        Chat, on_delete=models.CASCADE, related_name='flashcards')
    term = models.CharField(max_length=200)
    definition = models.TextField()
    context = models.TextField(blank=True)  # Context from conversation
    auto_generated = models.BooleanField(
        default=False)  # AI-generated vs user-created
    created_at = models.DateTimeField(auto_now_add=True)
    last_reviewed = models.DateTimeField(null=True, blank=True)
    review_count = models.IntegerField(default=0)
    confidence_level = models.IntegerField(
        default=0)  # 0-5 scale for spaced repetition
