...
class LangChainRAG:
    ...
    def build_index(self, file_paths_and_types, chat_id=None, rag_files_map=None):
        ...
        for i, (chunk, embedding) in enumerate(zip(self.chunks, embeddings_list)):
            ...
            if not rag_file:
                print(f"Warning: Could not find RAG file for chunk {i}, file_path: {file_path}")
        ...
        print(f"Successfully stored {len(chunk_objects)} chunks in PostgreSQL for chat {chat_id}")

    def retrieve_docs(self, query: str, chat_id=None) -> List[Document]:
