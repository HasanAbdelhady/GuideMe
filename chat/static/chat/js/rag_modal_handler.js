...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (!manageRagContextBtn || !ragModalOverlay || !ragModalContent || !closeRagModalBtn || !ragFilesListDiv || !noRagFilesMessage || !ragFileInput || !uploadRagFileBtn || !ragFileCountSpan || !ragUploadSection || !ragLimitReachedMessage) {
        console.warn("One or more RAG modal elements not found. RAG context management might not work.");
    ...
    function renderRAGFilesList(files) {
        if (!ragFilesListDiv || !noRagFilesMessage) {
            console.warn("RAG file list or no-files message element not found for rendering.");
            return;
        }
    ...
    function updateRAGFileLimitView() {
        if (!ragFileCountSpan || !ragUploadSection || !ragLimitReachedMessage || !ragFileInput || !uploadRagFileBtn) {
    ...
    async function fetchRAGFiles() {
        ...
        try {
            ...
            if(window.appendSystemNotification) window.appendSystemNotification("Could not load RAG files: " + error.message, "error");
            renderRAGFilesList([]); 
        }
    }

    async function handleRAGFileUpload() {
        if (window.isNewChat || !window.currentChatId || window.currentChatId === "new") return;
        if (!ragFileInput) {
        ...
        if (!file) {
            if(window.appendSystemNotification) window.appendSystemNotification("Please select a file to upload to RAG.", "warning");
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`/chat/${window.currentChatId}/rag-files/`, {
                method: "POST",
                body: formData,
                headers: { // Django often needs CSRF token for POST, even with FormData for AJAX
                    "X-CSRFToken": csrfToken 
                }
            });
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || `Failed to upload file (status: ${response.status})`);
            if (data.success && data.file) {
                if(window.appendSystemNotification) window.appendSystemNotification(`Successfully added '${data.file.name}' to RAG context.`, "info");
                fetchRAGFiles(); 
            } else {
            ...
            if(window.appendSystemNotification) window.appendSystemNotification(`Upload failed: ${error.message}`, "error");
        } finally {
            if (uploadRagFileBtn) {
                uploadRagFileBtn.disabled = false;
    ...
    async function handleDeleteRAGFile(fileId) {
        ...
        if (!csrfToken) {
            ...
            if(window.appendSystemNotification) window.appendSystemNotification("Action failed: Missing security token.", "error");
            return;
        }

        try {
            const response = await fetch(`/chat/${window.currentChatId}/rag-files/${fileId}/delete/`, {
                method: "DELETE",
                headers: {
                    "X-CSRFToken": csrfToken,
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.error || `Failed to delete RAG file (status: ${response.status})`);
            }
            // No JSON response expected for a successful DELETE often (204 No Content)
            if(window.appendSystemNotification) window.appendSystemNotification("Successfully removed file from RAG context.", "info");
            fetchRAGFiles(); 
        } catch (error) {
            console.error("Error deleting RAG file:", error);
            if(window.appendSystemNotification) window.appendSystemNotification(`Failed to remove file: ${error.message}`, "error");
        }
    }

    // Event Listeners for modal interaction
    if (manageRagContextBtn) manageRagContextBtn.addEventListener("click", openRAGModal);
    if (closeRagModalBtn) closeRagModalBtn.addEventListener("click", closeRAGModal);
    if (ragModalOverlay) {
        ragModalOverlay.addEventListener("click", function (event) {
            if (event.target === ragModalOverlay) closeRAGModal();
        });
    }
    if (uploadRagFileBtn) uploadRagFileBtn.addEventListener("click", handleRAGFileUpload);
