    ...
    async def execute(self, user_message: str, chat_context: Dict[str, Any]) -> ToolResult:
        try:
            ...
            if quiz_data and quiz_data.get('quiz_html'):
                ...
                error_msg = quiz_data.get(
                    'error', 'Failed to generate quiz') if quiz_data else 'Failed to generate quiz'
                return ToolResult(
                    success=False,
                    error=error_msg
                )
        ...
        try:
            ...
            for question_data in questions:
                try:
                    ...
                    logger.error(
                        f"Error saving individual question to bank: {db_error}", exc_info=True)
                    continue
    ...
    def _extract_questions_from_html(self, quiz_html: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, div in enumerate(question_divs):
                ...
                question_text = question_text_div.get_text(
                    strip=True) if question_text_div else f'Question {i + 1}'
