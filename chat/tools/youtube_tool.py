...
class YouTubeTool(BaseTool):
    ...
    def triggers(self) -> List[str]:
        return [
            "youtube", "video", "watch", "recommend", "tutorial", "learn more",
            "show me videos", "find videos", "educational content"
        ]
    ...
    async def execute(self, user_message: str, chat_context: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            response = await chat_service.get_youtube_agent_response(user_message, chat_history)
            
            logger.info(f"YouTube agent response type: {type(response)}")
            logger.info(f"YouTube agent response (first 200 chars): {str(response)[:200]}...")
            
            # Try to parse as JSON for video recommendations
            try:
                video_data = json.loads(response)
                logger.info(f"Successfully parsed JSON. Type: {type(video_data)}, Length: {len(video_data) if isinstance(video_data, list) else 'N/A'}")
            ...
            return ToolResult(
                success=False,
                error=f"YouTube tool error: {str(e)}"
            ) 