class MultiStepRegistration {
		...
		document.querySelector("form").addEventListener("keydown", (e) => {
			...
			if (e.key === "Tab" && !e.shiftKey && this.currentStep === 1) {
				...
				if (e.target === lastInput) {
					...
					setTimeout(() => {
						const firstStep2 = document.querySelector(
							"#step-2 input, #step-2 select, #step-2 button"
						);
	...
	showError(message) {
		...
		errorDiv.className =
			"step-error p-4 mb-4 text-red-200 bg-red-800 rounded-lg";
	...
	updateQuizPreference(radio) {
		...
		document
			.querySelector(`[data-rating="${radio.value}"]`)
		...
		document.querySelector(`[data-rating="${radio.value}"]`).textContent =
			ratingTexts[radio.value];
	}
	...
	initializeInterests() {
		...
		const createInterestTag = (name, id = null) => {
			...
			tag.className =
				"interest-tag px-3 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-sm text-white rounded-full flex items-center gap-2 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-blue-500/25 cursor-pointer";
			tag.innerHTML = `
                <span>${name}</span>
